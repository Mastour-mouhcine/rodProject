...
const JSalertAfterValidate = (status, message, type) => {
  var url = '/rodProject/index_validmails.html';
...
const initiateTableDisplay = (sheet_data) => {
  ...
  table_output +=
    ...
    '<button id="btn_validerMail" disabled class="btn btn-success" >Vérifier mails</button>\n' +
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          if(data.trim() === "New Records Created Successfully"){
            ...
            document.getElementById('btn_validerMail').disabled=true;
          }
          else{
