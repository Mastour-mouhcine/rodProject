 ...
 //Establishes the connection
    ...
    if ($result_seg != 'undefined') {
      ...
      if ($result_brand != 'undefined') {
        if($result_seg == 'undefined' && $string_sect == 'undefined' )
            $result_brand = " WHERE [Email] in(". $string_brand . ") ";
        else{
            $result_brand = " AND [Email] in(". $string_brand . ") ";
        }
      }
    $tsql= "SELECT 
    [First Name]
    ,[Last Name]
    ,[Email]
    FROM [dbo].[Import_Zoho_Contact_test] $string_seg $string_sect";
