...
$connectionOptions = array(
   ...
   foreach($dataArray as $item) {
      ...
      ,[CIBLAGE],[ciblage prox],[04/10/2021 consignes RC],[10/11/2021 consignes RC ciblages etc],[09/07/2021 consignes RC],[Suite 09/07/2021 consignes RC2],[Suite 09/07/2021 consignes RC3],[Remarques ],[Date],[Gestionnaire de compte ],[24/11/2021 consignes RC],[PUBLICATION RESEAUX SOCIAUX],[ID_Seg]
                     )
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; 

   $params = array($no,$statut,$priorite_call,$avtif,$dossier_nom,$date_der_contact,$ref,$ref_old,$titre_fr,$titre_nl,$comm_imm,$comm,$province,$region,$pays,$prix,$reg_ord_dans_list2,$sec_ord_list2,$sf,$sn,$reg_lc,$brok,$stop,$desc_fr,$desc_nl,$desc_en,$detail_fr,$detail_nl,$blacklist,$nom_doss,$cmp,$vat,$client,$mail,$mail2,$gsm,$tel,$tel2,$duree_cntr,$debut_cntr,$fin_contr,$t_contr,$commiss,$lang,$gmaps,$whise,$whise_ok,$mot_cle,$titre_z_s,$dans_z_s,$cible_rend,$cible_prom,$gros,$ciblage,$ciblage_pr,$consign_rc,$rc_etc,$rc,$rc2,$rc3,$remarque,$date,$gdc,$cons_rc,$pub_res_soc,$id_seg);    
