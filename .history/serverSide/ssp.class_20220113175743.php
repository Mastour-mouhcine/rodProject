...
class SSP {
	...
	static function order ( $request, $columns ) {
		...
		$order =  "ORDER BY ID OFFSET ".intval($request['start'])." ROWS FETCH NEXT ".intval($request['length'])." ROWS ONLY";
	}
		return $order;
	  }
	...
	static function simple ( $request, $sql_details, $table, $primaryKey, $columns ) {
		...
		// throw new Exception("SET NOCOUNT ON SELECT [".implode('], [', self::pluck($columns, 'db'))."] FROM $table $where $order $limit");
		// Main query to actually get the data
		$data = self::sql_exec( $db, $bindings,
		"SELECT [".implode('], [', self::pluck($columns, 'db'))."] FROM $table $where $order");
		
		//throw new Exception("SET NOCOUNT ON SELECT ".implode(", ", self::pluck($columns, 'db'))." FROM $table $where ");
	  // Data set length after filtering  the $where will update info OR will be blank when not doing a search
		...
		// $recordsFiltered = count($resFilterLength);
		$sql = "SELECT {$primaryKey} FROM $table $where $order";
		$params = array();
		$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
		$stmt = sqlsrv_query( $conn, $sql , $params, $options );
		$recordsFiltered = sqlsrv_num_rows( $stmt );
		...
		$resTotalLength = self::sql_exec( $db,"SELECT DISTINCT p.rows AS [Row Count] 
		FROM sys.partitions p
		INNER JOIN sys.indexes i ON p.object_id = i.object_id
								 AND p.index_id = i.index_id
		WHERE OBJECT_SCHEMA_NAME(p.object_id) != 'sys' AND OBJECT_NAME(p.object_id) = '$table'" );
	...
	static function sql_connect ( $sql_details )
	{
		try {
            ...
            $db = new PDO("sqlsrv:server = {$sql_details['host']},1433; Database = Data_Rods", $sql_details['user'],  $sql_details['pass']);
