...
var table=	$('#import_zoho_contact1').DataTable( {
    ...
    $.each($('div.dtsp-searchPane'), function(i, col) {
      $.each($('tr.selected', col), function(j, row) {
          ...
          segmentationData[i].push($('span:eq(0)', row).text());
          //console.log(JSON.stringify(segmentationData[i]));
             console.log('Column ' + i + ': ' + $('span:eq(0)', row).text())
        });
    });
    localStorage.setItem('segmentationData', JSON.stringify(segmentationData))
    // console.log(localStorage.getItem('segmentationData', JSON.stringify(segmentationData)));
