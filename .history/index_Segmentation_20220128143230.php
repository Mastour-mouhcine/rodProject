    ...
    $.each($('div.dtsp-searchPane'), function(i, col) {
      $.each($('tr.selected', col), function(j, row) {
          ...
          if (localStorage.getItem('segmentationData') === null) {
              localStorage.setItem('segmentationData', data);
          }
          let segmentationData = localStorage.getItem('segmentationData');

          // Init segsegmentationData
          segmentationData = Object.assign({}, data);
        if(i == 0){
            segmentationData.segmentation.push($('span:eq(0)', row).text());
        }
        else{
            segmentationData.secteur.push($('span:eq(0)', row).text());
        }
        console.log(segmentationData);
        localStorage.setItem('segmentationData', segmentationData)
