...
mycursor1.execute("SELECT Email FROM Seg_Input")
liste1=[]
...
#         )""")
mycursor1.execute("""TRUNCATE TABLE Mail_Seg """)

mycursor1.execute("""IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Mail_Seg' AND xtype='U')
          CREATE TABLE Mail_Seg (
...
for row in liste1:
    ...
    k=c.replace("'", "")
    mycursor1.execute('''
                INSERT INTO Mail_Seg (Email, Status)
                VALUES (?,?)
                ''',
                k,
                str(row[1])     
                )
mycursor1.execute('''UPDATE t2 SET t2.mail_status = t1.Status FROM dbo.Seg_Input t2 JOIN dbo.Mail_Seg t1 ON t2.Email = t1.Email; ''')
mydb.commit()
